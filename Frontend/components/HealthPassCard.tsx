...
export function HealthPassCard({
  ...
  const getStatusIcon = () => {
    switch (stats.status_color) {
      ...
      case "yellow":
        return <Clock className="text-amber-500" size={48} strokeWidth={2.5} />;
      case "red":
        return (
          <AlertCircle className="text-rose-500" size={48} strokeWidth={2.5} />
        );
      default:
        return <Award className="text-blue-500" size={48} strokeWidth={2.5} />;
    }
  };
  ...
  const getStatusBg = () => {
    switch (stats.status_color) {
      case "green":
        return "from-emerald-400 to-teal-500 shadow-emerald-200 dark:shadow-emerald-900/20";
      case "yellow":
        return "from-amber-400 to-orange-500 shadow-amber-200 dark:shadow-amber-900/20";
      case "red":
        return "from-rose-400 to-pink-500 shadow-rose-200 dark:shadow-rose-900/20";
      default:
        return "from-blue-400 to-indigo-500 shadow-blue-200 dark:shadow-blue-900/20";
    }
  };
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-br ${getStatusBg()} opacity-10 dark:opacity-20 blur-[100px] rounded-full`}
      />
      ...
      <div className="relative z-10 p-10">
        <div className="flex justify-between items-start mb-12">
          <div>
            ...
            <h3 className="text-5xl font-black text-slate-800 dark:text-white tracking-tighter mb-1 leading-none">
              {dependent.name}
            </h3>
            <p className="text-slate-400 font-bold text-sm tracking-wide">
              Dependent ID: {dependent.id.slice(0, 8)}
            </p>
          </div>
          ...
          <div className="flex flex-col justify-between py-2 space-y-6">
            <div className="bg-white/60 dark:bg-slate-700/60 p-8 rounded-[2.5rem] border border-white/50 dark:border-slate-600/30 shadow-sm">
              ...
              <div className="flex items-end gap-3">
                ...
                <div
                  className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${stats.health_score >= 80 ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400"}`}
                >
            ...
            <div className="bg-white/60 dark:bg-slate-700/60 p-8 rounded-[2.5rem] border border-white/50 dark:border-slate-600/30 shadow-sm">
              ...
              <div className="flex items-end gap-3">
                ...
                <span className="text-sm font-bold text-slate-400 pb-1 italic">
        ...
        {next_due.name && (
          ...
          >
            ...
            <div>
              <p className="text-[10px] font-black text-blue-500 dark:text-blue-400 uppercase tracking-[0.2em] mb-1">
                Upcoming Priority
              </p>
              ...
              <p className="font-bold text-slate-400 dark:text-slate-500">
                Scheduled for{" "}
                {new Date(next_due.date!).toLocaleDateString("en-US", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                })}
        ...
        <div className="flex flex-col sm:flex-row gap-5">
          ...
          <button
            className={`flex-[1.5] bg-gradient-to-r ${getStatusBg()} h-20 rounded-[1.5rem] font-black text-white transition-all flex items-center justify-center gap-3 shadow-2xl active:scale-95 group`}
          >
            <Share2
              size={24}
      ...
      <p className="text-center pb-10 text-[9px] font-black text-slate-400 uppercase tracking-[0.4em] opacity-50">
        Vaxi Babu Certified Health Record System
      </p>
    </motion.div>
  );
}
